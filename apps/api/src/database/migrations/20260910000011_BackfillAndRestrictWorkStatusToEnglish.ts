...
/**
 * `works.status` (`WorkStatus` in packages/types/src/enums.ts) is a plain
 * `varchar(50)` with no CHECK constraint, currently holding the PT-BR values
 * `pendente` / `analise` / `em_analise` / `registrado` / `ativo` / `inativo` /
 * `rejeitado` / `arquivado`. This project's deploy model is atomic (schema +
 ...
 *   3. Add `chk_works_status` CHECK constraint restricting the column to the
 *      English enum values.
 */
export class BackfillAndRestrictWorkStatusToEnglish20260910000011
  implements MigrationInterface
{
  name = 'BackfillAndRestrictWorkStatusToEnglish20260910000011';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    for (const [pt, en] of this.ptToEn) {
      ...
      console.log(
        `[BackfillAndRestrictWorkStatusToEnglish] works.status '${pt}' -> '${en}': ${affected} row(s)`,
      );
    }
    ...
    if (invalidRows.length > 0) {
      ...
      throw new Error(
        `BackfillAndRestrictWorkStatusToEnglish20260910000011: cannot add CHECK constraint, ` +
          `"works" contains rows with unexpected status values after backfill: [${invalidValues}]. ` +
    ...
    // added below.
    await queryRunner.query(`ALTER TABLE "works" ALTER COLUMN "status" SET DEFAULT 'pending'`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ALTER TABLE "works"
      DROP CONSTRAINT IF EXISTS "chk_works_status"
    `);
    await queryRunner.query(`ALTER TABLE "works" ALTER COLUMN "status" SET DEFAULT 'pendente'`);

    for (const [pt, en] of this.ptToEn) {
      await queryRunner.query(
        `UPDATE works SET status = $1, updated_at = now() WHERE status = $2`,
        [pt, en],
      );
    }
  }
